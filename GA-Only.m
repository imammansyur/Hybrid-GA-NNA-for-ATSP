function GA_Only_and_TSPLIB()
    ...
    subfolderPath = fullfile('outputs', timestamp+'-GA');
    if ~exist(subfolderPath, 'dir')
        mkdir(subfolderPath);
    end
    csvFile = fullfile(subfolderPath, 'results_' + timestamp + '-GA.csv');
    
    % Write CSV header
    header = {'Data', 'GA_Distance', 'Runtime', ...
    ...
    for w = 1:length(dataList)
        ...
        tic;
        [bestRoute, bestDist, bestFitnessOverTime, GAParameters] = solverTSP_GA_with_NN(localDist);
        runtime = toc;
        ...
        if ~useTSPLIB
            plotRoutes(localPlaces, fullGARoute, dataName, subfolderPath, timestamp);
        end
...
function [bestRoute, bestDist, bestFitnessOverTime, params] = solverTSP_GA_with_NN(distMatrix)
...
function plotConvergence(fitness, wilayahName, subfolderPath, timestamp)
    ...
    title(['GA Convergence - ', wilayahName]);
